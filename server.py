...
def seed_data():
    ...
    if c.execute("SELECT COUNT(*) FROM movies").fetchone()[0] == 0:
        ...
        c.executemany('INSERT INTO ads (title,creative_url,target_category,duration_sec,token_reward,created_at) VALUES (?,?,?,?,?,?)', [
            ("Cinema Cloud", "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?w=600", "tech", 5, 3, now),
...
class Agent:
    ...
    def send_outreach(lead_id):
        ...
        if RESEND_KEY:
            try:
                requests.post("https://api.resend.com/emails", headers={"Authorization":f"Bearer {RESEND_KEY}"}, json={"from":"CinemaAgent <onboarding@resend.dev>","to":lead[1],"subject":"Partnership: Cinema-Live","html":"<p>Hi team,<br>Cinema-Live offers compliant, high-engagement ad syncs. Interested in a trial?</p>"})
            ...
            c.execute("UPDATE advertiser_leads SET outreach_sent=1,status='contacted' WHERE id=?",(lead_id,)); log("outreach","queued",f"Would email {lead[1]}")
